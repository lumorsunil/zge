...
pub fn DrawSystem(comptime config: ZGEConfig) type {
    return struct {
        screen: *const Screen,
        reg: *ecs.Registry,
        ...
        const CollisionContainer = config.CollisionContainer();
        ...
        pub fn init(allocator: Allocator, reg: *ecs.Registry, screen: *const Screen) @This() {
            return .{
                .screen = screen,
                .reg = reg,
        ...
        pub fn bind(self: *@This()) void {
            self.reg.onConstruct(RigidBody).connectBound(self, @This().ensureDrawOrderCapacity);
        }

        fn unbind(self: *@This()) void {
        ...
        fn drawCollisionContainer(self: *@This()) void {
            const cc = self.reg.singletons().get(CollisionContainer);

            if (config.getDynamicBodiesAlgorithm() == .rTree) {
                if (@hasField(@TypeOf(cc.tree), "root")) {
                    self.drawPage(cc.tree.root, 1, cc.tree.height);
                }
            } else {
                if (@hasDecl(@TypeOf(cc.tree), "getRoot")) {
                    self.drawPageQT(cc.tree.getRoot(), 0);
                }
            }
        }

        fn drawPageQT(self: *@This(), maybePage: ?*CollisionContainer.Tree.Page, level: usize) void {
        ...
        fn drawPage(self: *@This(), page: *CollisionContainer.Tree.Page, level: usize, height: usize) void {
        ...
        fn drawNode(self: *@This(), node: CollisionContainer.RTree.Node, level: usize, height: usize) void {
            switch (node) {
        ...
        fn drawEntry(self: *@This(), entry: CollisionContainer.RTreeEntry) void {
            self.drawAabb(CollisionContainer.entryAabb(entry), ccEntryColor, 1);
        }

        pub fn drawAabb(_: *@This(), aabb: AABB, color: rl.Color, thickness: f32) void {
            const p = aabb.tl;
        ...
        fn minYComparer(self: *@This(), a: ecs.Entity, b: ecs.Entity) bool {
        ...
        fn drawOrderFilterFn(self: *@This(), entity: ecs.Entity) bool {
            const invisible = self.reg.tryGet(Invisible, entity);

            return invisible == null;
        }

        fn drawOrder(self: *@This()) []const ecs.Entity {
        ...
        pub fn drawShape(self: *@This(), rb: *RigidBody) void {
            switch (rb.s.shape) {
        ...
        pub fn drawCircle(_: *@This(), circle: Circle, rb: *RigidBody) void {
            const center = rb.aabb.center();

            rl.drawCircleLinesV(V.toRl(center), circle.radius, rl.Color.white);
        }

        pub fn drawRectangleSolid(_: *@This(), aabb: AABB, color: rl.Color) void {
        ...
        pub fn drawRectangle(self: *@This(), _: Rectangle, rb: *RigidBody) void {
            ...
            const cc = self.reg.singletons().get(CollisionContainer);
            const color = if (config.getDynamicBodiesAlgorithm() == .quadTree)
                if (cc.tree.tree.isEntryInTree(rb.key)) rl.Color.white else rl.Color.red
            else
                rl.Color.white;
        ...
        fn drawDebugBorder(self: @This()) void {
            const lw = 4;
            const lo = V.all(lw / 2);
        ...
        pub fn drawText(_: *const @This(), text: []const u8, position: Vector, fontSize: f32, color: rl.Color) void {
